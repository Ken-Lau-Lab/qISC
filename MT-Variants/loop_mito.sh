...
do
	echo ${R1[i]}
	#echo $i
	zgrep --no-group-separator -A 0 -B 1 $1 ${R1[i]} > ./Data/$(basename ${R1[i]} _R1_001.fastq.gz)_R1.txt
	zgrep --no-group-separator -A 0 -B 1 $2 ${R2[i]} > ./Data/$(basename ${R2[i]} _R2_001.fastq.gz)_R2.txt
	python3 ./extract_from_reads_mito.py $(basename ${R1[i]} _R1_001.fastq.gz) $3
	echo $(basename ${R1[i]} _R1_001.fastq.gz) completed
done
