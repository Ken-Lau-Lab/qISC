...
input = '5607-YX-24'
BC_mito <- read.table(file = 'INUSE_BC_Mito_human.txt', header = T, fill = F)
...
for (gene in mito_gene){
  ...
  filenames = Sys.glob(file.path('output_mito', paste0(input,'*',gene,"\\.csv")))
  ...
  if (nrow(data_mut)> 0){
    ...
    data_mut$Cell<- paste0(data_mut$Cell, sep="-", input)
    output<- data_mut[, c(3,2,6,7)]
    write_csv(output,paste0("MMI/",gene,'_',input,'.csv'))
  }
  }
...
filenames = Sys.glob(file.path('MMI',paste0('*',input,'.csv')))
...
save_output = paste0(input,'_mitoBC_table.txt')
write.table(data, file = save_output, sep = "\t", row.names = F,col.names = T)
